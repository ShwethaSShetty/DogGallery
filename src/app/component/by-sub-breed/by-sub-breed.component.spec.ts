...
describe('BySubBreedComponent', () => {
  ...
  beforeEach(async () => {
    ...
    mockDogBreedServiceService = jasmine.createSpyObj<DogBreedServiceService>(DogBreedServiceService.name, ['getBreedList','getSubBreedList','getRandomDogImage','getDogImagesByBreed']);


    await TestBed.configureTestingModule({
      imports:[BySubBreedComponent,NoopAnimationsModule],
      providers: [
        provideHttpClient(),
  ...
  beforeEach(() => {
    ...
    mockDogBreedServiceService.getSubBreedList.and.returnValue(of({
      status: 'success',
      message: ['shepherd']
    }));
  ...
  it('should get sub breed list on breed selection', () => {
    component.onBreedSelection('bulldog');
    expect(mockDogBreedServiceService.getSubBreedList).toHaveBeenCalledWith('bulldog');
  });

  it('should set sub breed name on sub breed selection', () => {
    const subBreedName = 'terrier';
    component.onSubBreedSelection(subBreedName);
    expect(component.subBreedName).toBe(subBreedName);
  });
